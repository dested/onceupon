...
## 2026-09-23 — Talking intro replaces the one-card tutorial
Sal: "we need an onboarding intro that talks to you ... a few slides, all with the cool animation, predraw some cool ones". The old tutorial never showed: `.story-studio { position: relative }` (unlayered) beat Tailwind's `fixed`, so the overlay rendered below the story screen, off-screen. New `IntroScreen`: five picture-book pages (Tell a story your way / Say whatever is in your head and watch it draw / Change your mind anytime, the crayon keeps up / When you're done, say "The End" / Ready? Tap the mic and start), each drawn live by the real engine from a Sonnet-authored ops record while an OpenAI TTS (coral) voice reads it; Next/Back/dots/swipe, hear-again, skip; once on first launch, again from the story menu ("How it works") and Grown-ups. Ops parser now accepts a trailing `Z` on `poly` (the model wrote one for the rocket body and the whole shape was dropped). Screen stays awake while listening or replaying (`boot.ts`, from store state).
Touched: src/tutorial/{IntroScreen.tsx,intro.ts,intro-copy.ts,intro.css,intro-slides.json,intro/*.mp3} (Tutorial.tsx, cues.ts removed), scripts/{author-intro,render-coach}.ts, src/llm/ops-dsl.ts, src/app.tsx, src/boot.ts, src/ui/{StoryScreen,ParentArea}.tsx, cliffnotes.md

## 2026-09-23 — Safe area everywhere, keyboard avoidance, modals that fit
iPad test: only the story screen respected the safe area, the keyboard covered every input, modals looked off. One inset source (`--sat/--sar/--sab/--sal` = max(env(), shell `--shell-inset-*`)) now pads the studio, shelf (`.screen-safe`), replay chrome (toolbar, filmstrip, scrubber, captions, subtitles), export card and every overlay. `src/ui/viewport.ts` turns visualViewport into `--kb`/`--vv-top`/`html.kb-open`: modals (`.modal-overlay` + `.modal-card`, `.new-story-dialog`) re-center above the keyboard and scroll inside, the typing tray rides on the keyboard, the focused field scrolls into view in its card; `maximum-scale=1` + 16px field floor stop iOS focus zoom. Verified in an iframe harness at 1180x820 / 820x1180 / 390x844 with fake insets and a fake keyboard; not yet on a real iPad keyboard.
Touched: index.html, src/main.tsx, src/styles/app.css, src/ui/{viewport.ts,Bookshelf,ReplayScreen,Filmstrip,VideoExport,KeyGate,SettingsPanel,ParentArea,Paywall,ShareCard}.tsx, ui.md

## 2026-09-23 — iPad shell: bridge alive, fast boot, save to Photos, last native build's modules
On the iPad the studio thought it was on the web (downloads, web shop navigation, 12 s splash): `WKAppBoundDomains` in Info.plist makes WebKit drop user scripts and message handlers on non-app-bound navigations, so `window.ReactNativeWebView` never existed. Removed it; `hasBridge()` now keys off `shell=native` plus the injected `__onceuponShell` and requests wait up to 3 s for a late ReactNativeWebView; interactive bridge calls (StoreKit, share, Photos) get a 10 min timeout instead of 20 s. Boot: no network probe before the first render (probe runs in parallel, falls back to the bundled copy), splash backstop 3 s, `initDevice` no longer awaited before first paint (`api()` waits on `deviceTokenSettled()`). Exports save to Photos (`media.saveVideo`, add-only permission) with Share as second step / fallback. The WebView only shows `/app`; the rest of the origin opens in an in-app Safari sheet (`expo-web-browser`); no web-shop links or navigation in the app (`APP_SHELL`). Last native build, so also installed and bridged: expo-audio (`audio.mode`, playback at launch), keep-awake (`awake.set`, studio `setAwake`), screen-orientation, store-review, localization (locale/region in `device.info`), notifications (plugin + `notify.permission`, never auto-prompts), expo-speech-recognition (`speech.start/stop` + `speech.*` events), safe-area insets as `--shell-inset-*` vars + `insets` event, system-ui paper root. Needs a new native build to take effect.
Touched: apps/mobile/{app.config.ts,package.json,src/App.tsx,src/StudioWebView.tsx,src/links.ts,src/studio-source.ts,src/bridge/**}, packages/shared/src/bridge.ts, src/boot.ts, src/backend/{bridge,config,share,awake,device,api,purchases}.ts, src/ui/{VideoExport,Paywall,ParentArea}.tsx

## 2026-09-23 — Typed words are metered; first voice clip replays
Typed messages were free minutes in hosted mode. New `session.typed` ({sessionId, text}): the server prices each message as talking time from the text, `max(3, ceil(words / 2.5))` s (rule in `packages/shared/src/typed.ts`), debits the ledger (note `typed`), adds to `StorySession.chargedSec` and the new `typedSec` column (listenedMs stays mic time), and returns remaining/exhausted like a beat; exhausted ends sleepy. The meter reopens a session the stale-session job closed (`not_found` → start + retry once), so a long pause no longer breaks metering or the relay. Admin device ledger shows Typed. Replay: a voiced replay started at the first event, which is seconds after the first clip began (the clip starts when the mic opens, the words event only once recognized), so clip 1 was always entered mid-way — first words cut, and a seek into an unloaded MediaRecorder file at replay start, which WKWebView loses. Voiced replays (and the mp4) now start up to 8 s earlier (`voiceLeadMs`); VoicePlayer loads every clip to metadata before play, defers early seeks to `loadedmetadata`, stops re-seeking a playing clip every step, never restarts an ended clip, and is re-synced every 200 ms; recorder chunks are per span.
Touched: packages/shared/src/{api,typed,index}.ts, apps/web/prisma/schema.prisma, apps/web/server/{sessions,admin-queries}.ts, apps/web/server/app-handlers/core.ts, apps/web/src/app/admin/ledger.tsx, src/backend/meter.ts, src/story/{session,voice,replay}.ts, src/export/mp4.ts

## 2026-09-23 — First production deploy + ears fallback
Sal pushed. The first Drydock deploy failed because the project had been converted from static and no Postgres/DATABASE_URL existed; replayed the create-time DB block through Drydock's helpers, re-applied, re-ran the workflow. https://onceupon.dested.com is live: site, `/app/`, `/admin`, and register → config → session.start verified on production. The Deepgram key is refused by `/v1/auth/grant` (403, key lacks the scope), which left sessions with no ears, so the server now falls back to the other vendor (`mintEarsWithFallback`); OpenAI live transcribe mints. A Deepgram key with member scope re-enables the cheaper ears. Removed stray root `app.json`/`eas.json` left by an `eas init` run from the wrong directory.
Touched: apps/web/server/{ears,sessions}.ts, updates.md

## 2026-09-22 — Deploy wiring: Drydock ssr, App Store Connect app + consumables, EAS project
Sal: "commit everything, do the drydock fully, create the project and stuff in apple and expo." Committed the launch platform (rebased onto Drydock's regenerated Dockerfile/workflow for rootDir apps/web). Drydock project `onceupon` switched to ssr + Postgres through the portal API (`apps/web/scripts/drydock-apply.ts`: config + SSM env). Apple, driven in the quickgame Chrome: App ID com.quickgame.squiggletale registered (In-App Purchase on), App Store Connect app Squiggletale created (id 6815029998, SKU squiggletale), three consumables with pricing, all-country availability and English localization (pack40 $9.99, pack120 $19.99, pack400 $39.99), App Store Server Notifications production + sandbox URL → `/api/apple/notifications`. EAS project @quickgame/squiggletale already existed. GitHub push protection caught the hosted build inlining the VITE_ keys into `studio.html` (local commits only); hosted builds now expose only VITE_HOSTED/VITE_API_ORIGIN and the commits were squashed clean.
Touched: apps/web/scripts/drydock-apply.ts (new), apps/web/.env.example, vite.config.ts, Dockerfile + drydock.yaml + .github/workflows/drydock.yml (Drydock-managed), cliffnotes.md
