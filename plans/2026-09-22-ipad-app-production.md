...
- Landscape-first layout, safe areas, hardware keyboard for the typed fallback, a parent settings area behind the gate (balance, buy, redeem a gift, buy on the web, restore, privacy). The zero-balance screen looks like the finale card ("the crayon needs more minutes"), pre-selects the last pack bought, and offers restore and redeem underneath.
- Interruptions: phone calls, Siri, backgrounding close the socket and pause the meter; resume cleanly. Offline: the story screen says the crayon needs the internet; the bookshelf and replay (and the finale) work offline, which is the road-trip promise alongside the 400 pack.
...
Pages: landing (an embedded live replay is the hero, the pitch is the product), how it works (the tutorial story), pricing (packs and the free allowance in plain words), shop (Stripe checkout for packs, signed in to a device account via a link from the app) and gift (buy a code, printable card, "how to redeem"), FAQ, support/contact, terms of service, privacy policy with a children's privacy section (what the mic audio is used for, that it is not stored, which vendors process it, how to delete a story or a device's data), account and data deletion request page (Apple requires one when any data is held), and the `/s/<id>` share pages. Plus the API: Sonnet stream relay, STT token minting, ledger and free grants, IAP validation, Stripe webhooks, gift-code mint and redeem, story upload and unpublish.
