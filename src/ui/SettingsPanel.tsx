...
export function SettingsPanel() {
  ...
  const soundToggle = (
    <label className="font-hand mb-5 flex items-center gap-3 text-lg" htmlFor="setting-sound">
      <input
        id="setting-sound"
        type="checkbox"
        className="accent-crayon-green h-5 w-5"
        checked={settings.sound}
        onChange={(e) => update((s) => ({ ...s, sound: e.target.checked }))}
      />
      Crayon sounds
    </label>
  )
  ...
  const moderationToggle = (
    <label className="font-hand mb-5 flex items-center gap-3 text-lg" htmlFor="moderation">
      ...
      />
      Kid-safe moderation (skip not-for-kids beats, mask rude words). Prompt change applies to the
      next new story.
    </label>
  )
      ...
      <div className="grid gap-3">
        ...
        <div>
          ...
          <input
            ...
            placeholder="paste a key, stays in this browser only"
            className={field}
            onChange={(e) => update((s) => ({ ...s, keys: { ...s.keys, deepgram: e.target.value } }))}
          />
        </div>
      </div>
      <p className="font-hand text-ink-soft mt-4 text-base">
        Keys are used straight from your browser and stay on this device only. Press the backtick key
        for the debug panel.
      </p>
    </>
  )
    ...
    <div className="bg-ink/30 absolute inset-0 z-40 grid place-items-center" onClick={close}>
      <PaperCard className="relative max-h-[92dvh] w-[36rem] max-w-[calc(100vw-32px)] overflow-y-auto">
        <div onClick={(e) => e.stopPropagation()}>
          ...
          <h2 className="font-scrawl mb-4 text-3xl">{hosted ? 'Grown-up settings' : 'Settings'}</h2>

          {hosted ? (
            <>
              {micPicker}
              {soundToggle}
              {/* Developer: the bring-your-own-key controls, never shown in a hosted production build. */}
              {import.meta.env.DEV && (
                <details className="mt-2">
                  <summary className="font-hand mb-3 cursor-pointer text-lg text-ink-soft">Developer</summary>
                  {drawingBrain}
                  {moderationToggle}
                  {dialectPicker}
                  {earsPicker}
