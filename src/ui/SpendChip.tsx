...
export function SpendChip() {
  ...
  return (
    <div
      className="absolute top-[4.6rem] right-4 rounded-lg bg-paper/85 px-2 py-0.5 font-hand text-sm text-ink-soft"
      data-testid="spend"
      title={`${spend.input.toLocaleString()} in (${spend.cached.toLocaleString()} cached) / ${spend.output.toLocaleString()} out tokens`}>
      ${usd.toFixed(3)}
      {spend.unpriced > 0 ? `+? (${known}/${spend.calls} priced)` : ''} · {spend.calls} calls · {Math.round(avg)}ms to first stroke
      {spend.audioMs > 0 ? ` · ears ~$${earsUsd.toFixed(3)} (${(spend.audioMs / 60000).toFixed(1)} min)` : ''}
    </div>
  )
}
