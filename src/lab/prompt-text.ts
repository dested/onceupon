/**
 * Pure prompt-editing helpers with no browser dependency, so both the lab page and the node-side
 * `scripts/lab-agent.ts` CLI can import them: apply an editor find/replace patch, validate the result
 * against the frozen grammar and example story, diff two versions, and slice a section by header.
 * `countPromptTokens` stays in prompt-tools.ts because it needs the Anthropic client.
 */
