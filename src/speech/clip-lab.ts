import { z } from 'zod'
import { isLiveModel, pcmToWav, SAMPLE_RATE } from './openai-realtime'
...
const evSchema = z.object({ type: z.string(), delta: z.string().optional(), transcript: z.string().optional(), error: z.object({ message: z.string().optional() }).optional() })
...
function viaRealtime(pcm: Int16Array, model: string, apiKey: string): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    const socket = new WebSocket('wss://api.openai.com/v1/realtime?intent=transcription', ['realtime', `openai-insecure-api-key.${apiKey}`])
    let text = ''
    let settle = 0
    let hard = 0
    const finish = (): void => {
    ...
    socket.onmessage = (m) => {
      ...
      if (e.type === 'session.updated') {
        ...
        for (let i = 0; i < pcm.length; i += step) {
          ...
          socket.send(JSON.stringify({ type: 'input_audio_buffer.append', audio: toBase64(new Uint8Array(part.buffer, part.byteOffset, part.byteLength)) }))
        }
        const silence = new Int16Array(SAMPLE_RATE)
        socket.send(JSON.stringify({ type: 'input_audio_buffer.append', audio: toBase64(new Uint8Array(silence.buffer)) }))
...
export async function transcribeClip(pcm: Int16Array, model: string, apiKey: string): Promise<ClipResult> {
  const t0 = performance.now()
  try {
    const text = isLiveModel(model) ? await viaRealtime(pcm, model, apiKey) : await viaRest(pcm, model, apiKey)
    return { model, text, ms: Math.round(performance.now() - t0), error: null }
  } catch (e: unknown) {
    return { model, text: '', ms: Math.round(performance.now() - t0), error: e instanceof Error ? e.message : String(e) }
  }
}

export function compareClip(pcm: Int16Array, apiKey: string, models: readonly string[] = CLIP_LAB_MODELS): Promise<ClipResult[]> {
