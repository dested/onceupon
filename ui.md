...
- **Safe area:** paper and backgrounds bleed full-screen; every control and readable thing pads by `--sat/--sar/--sab/--sal` (app.css `:root`: `max(env(), --shell-inset-*)`). Full screens use `.story-studio` or `.screen-safe`; floating chrome uses `calc(<gap> + var(--sa*))` (replay toolbar, filmstrip, scrubber, captions, export card). Never `env()` directly.
- **Modals:** `.modal-overlay` (fixed, full-bleed backdrop, padded by insets + keyboard; component sets z + backdrop colour) around a `PaperCard` with `.modal-card` (max-height of the visible box, scrolls inside, momentum, contained overscroll). Native `<dialog>`s use `.new-story-dialog`, which does the same. The document never scrolls: anything taller than the screen scrolls inside its own box.
- **Keyboard:** the WebView never resizes for it. `src/ui/viewport.ts` (installed in main.tsx) turns `visualViewport` into `--kb`, `--vv-top` and `html.kb-open`; overlays re-center above it, bottom trays go `position: fixed; bottom: calc(var(--kb) + 12px)` under `.kb-open`, and the focused field is scrolled into view inside its scroll box. Fields are 16px or larger (iOS zooms below that). Dev: `__viewport.fakeKeyboard(px)` / `fakeInsets({...})`.
